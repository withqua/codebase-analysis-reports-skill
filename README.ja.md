...
1. `CODEBASE_ANALYSIS_HUMAN.html` — 標準化されたデザイン（ダーク/ライトモード、レスポンシブ、コードハイライト、Mermaid図、印刷最適化）の自己完結型HTML分析レポート
2. `CODEBASE_ANALYSIS_AGENT.md` — 後続のAIエージェントが引き継げる実行中心の markdown handoff(引き継ぎ) レポート
...
- 人間用レポート(HTML)は30KB以下、エージェント用レポート(MD)は7KB以下（`wc -c` 基準）。
- 根拠のない問題やリスクは書きません。
...
```text
/path/to/repo を分析してください。目的は、このコードベースのプラグイン構造と再利用できる部分を把握することです。その目的に合わせて、人間用レポートとAIエージェント用引き継ぎレポートを作ってください。
```

## ファイル

- `SKILL.md` — 実際の Hermes Agent スキル
- `references/html-template.html` — 人間用HTMLレポートテンプレート（デザイン仕様）
- `README.md` — 韓国語ガイド
